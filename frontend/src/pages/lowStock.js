...
function exportToExcel(data, headers, filename = "LowStockReport.xlsx") {
    // Prepare table
    let csv = '';
    csv += headers.join('\t') + '\n';
    data.forEach(row => {
...
const LowStock = () => {
    ...
    const [LowStock, setProducts] = useState([]);
    ...
    useEffect(() => {
        ...
        const results = LowStock.filter((stock) =>
    ...
    //     exportToExcel(dataToExport, tableHeaders, "LowStockReport.xls");
    // };

    return (
        <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <h2 style={{ margin: 0 }}>Low Stock List <span style={{ fontWeight: 400, fontSize: 26, color: '#888' }}></span></h2>
