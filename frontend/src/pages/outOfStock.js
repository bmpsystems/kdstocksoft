...
function exportToExcel(data, headers, filename = "OutOfStockReport.xlsx") {
    // Prepare table
    let csv = '';
    csv += headers.join('\t') + '\n';
    data.forEach(row => {
...
const OutOfStock = () => {
    ...
    const [OutOfStock, setProducts] = useState([]);
    ...
    useEffect(() => {
        ...
        const results = OutOfStock.filter((stock) =>
    ...
    //     exportToExcel(dataToExport, tableHeaders, "OutOfStockReport.xls");
    // };

    return (
        <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <h2 style={{ margin: 0 }}>Out of Stock List <span style={{ fontWeight: 400, fontSize: 26, color: '#888' }}></span></h2>
