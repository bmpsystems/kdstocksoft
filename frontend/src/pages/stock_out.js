...
const StockOut = () => {
  ...
  const [forceEnableTopBillFields, setForceEnableTopBillFields] =
    useState(false);
  ...
  useEffect(() => {
    if (companyInputFocused && !selectedCompany && companyQuery.length >= 3) {
      const fetchSuggestions = async () => {
        try {
          const response = await axios.get(
            `https://kdstocksoft.onrender.com/company-search?query=${companyQuery}`,
          );
  ...
  useEffect(() => {
    if (
      ...
      const fetchSuggestions = async () => {
        try {
          const response = await axios.get(
            `https://kdstocksoft.onrender.com/stock-search?query=${productQuery}`,
          );
  ...
  const openEditModal = (stockIn) => {
    if (stockIn.Active === 1) {
      ...
      const newBillDetails = {
        ...
        invoiceDate: stockIn.Invoice_Date
          ? stockIn.Invoice_Date.split("T")[0]
          : "",
      ...
      const prodDetailsArr = Array.isArray(stockIn.prod_details)
        ? stockIn.prod_details
        : [];
      ...
      setOriginalProductIds(
        prodDetailsArr.map((prod) => String(prod.Prod_Id || prod.Id)),
      );
      ...
      if (
        newBillDetails.deliveryChallan &&
        newBillDetails.deliveryChallan.trim() !== "" &&
        newBillDetails.purpose &&
        newBillDetails.purpose.trim() !== ""
      ) {
  ...
  const normalizeBillDetails = (details) => {
    return {
      ...details,
      invoiceNo:
        details.invoiceNo && details.invoiceNo.trim() !== ""
          ? details.invoiceNo
          : null,
      invoiceDate:
        details.invoiceDate && details.invoiceDate.trim() !== ""
          ? details.invoiceDate
          : null,
      deliveryChallan:
        details.deliveryChallan && details.deliveryChallan.trim() !== ""
          ? details.deliveryChallan
          : null,
      purpose:
        details.purpose !== undefined && details.purpose !== null
          ? details.purpose
          : "",
    };
  };
  ...
  const handleSubmit = async () => {
    ...
    const hasChallanAndPurpose =
      !!billDetails.deliveryChallan && !!billDetails.purpose;
    if (
      (hasInvoice && hasChallanAndPurpose) ||
      (!hasInvoice && !hasChallanAndPurpose)
    ) {
      alert(
        "Please fill either Invoice No or both Challan and Purpose (not both, not neither).",
      );
      return;
    }
    ...
    try {
      if (isEdit && editStockInId) {
        ...
        const payload = {
          ...
          Purpose:
            normalizedBill.purpose !== undefined &&
            normalizedBill.purpose !== null
              ? normalizedBill.purpose
              : "",
        ...
        await axios
          .put(`${API_URL}/${editStockInId}`, payload)
          .then((response) => {
            setShowModal(false);
          ...
          .catch((err) => {
            ...
            if (
              ...
              err.response.data.details.startsWith("Insufficient")
            ) {
              Swal.fire({
                icon: "error",
                title: "Submit Failed",
      ...
      if (
        ...
        err.response.data &&
        err.response.data.error &&
        err.response.data.error.startsWith("Insufficient")
      ) {
        Swal.fire({
          icon: "error",
          title: "Submit Failed",
      ...
      alert(
        isEdit ? "Failed to update stock out." : "Failed to save stock out.",
      );
    }
  };
  ...
  const toggleActive = async (id) => {
    try {
      const res = await axios.post(`${API_URL}/toggle`, { id });
          ...
          {currentData && currentData.length > 0 ? (
            currentData.map((product) => (
              <tr key={product.Id}>
                ...
                <td>{product.Make}</td>
                <td>
                  {product.Total_Quantity + " " + product.prod_details[0].Unit}
                </td>
                  ...
                  <Form.Check
                    ...
                    onChange={async () => {
                      ...
                      if (window.confirm(confirmMessage)) {
                        ...
                        setActiveStates((prev) => ({
                          ...prev,
                          [product.Id]: updatedProduct.Active === 1,
                        }));
            ...
            <Col md={4}>
              <Form.Group style={{ position: "relative" }}>
                ...
                <Form.Control
                  ...
                  onBlur={() =>
                    setTimeout(() => setCompanyInputFocused(false), 150)
                  }
