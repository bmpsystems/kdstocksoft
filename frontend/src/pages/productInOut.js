...
const ProductInOut = () => {
  ...
  const handleSelectProduct = (product) => {
    let displayLabel = product.Model_no || "";

    setSearchTerm(displayLabel);
    setSelectedLabel(displayLabel);
    setShowDropdown(false);
    setDropdownWasSelected(true);

    // Exact Product Id search
    fetchTxnRows(product.Id);
  };

  // Movement fetcher with any string
  const fetchTxnRows = async (prodId) => {
    if (!prodId) return;
    ...
    try {
      const res = await axios.get(
        "https://kdstocksoft.onrender.com/product-movement-search",
        {
          params: { prodId },
        },
      );
    ...
    >
      ...
      {(selectedLabel || searchTerm.length >= 3) && (
        <div style={{ marginBottom: 16, color: "#1976d2", fontWeight: 500 }}>
          Showing Txn details for:{" "}
          <span style={{ color: "#333" }}>{selectedLabel || searchTerm}</span>
        </div>
      )}
            ...
            >
              ...
              <tbody>
                {txnRows.map((row, idx) => (
                  <tr
                    key={idx}
                    style={{ background: idx % 2 === 1 ? "#eef3ff" : "#fff" }}
                  >
                    <td
                      style={{
                        padding: 8,
