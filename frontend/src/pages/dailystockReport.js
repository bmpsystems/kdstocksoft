...
const DailyStockReport = () => {
  ...
  // -------------------- NEW: Stock Summary Sheet Helper ----------------------
  const addStockSummarySheet = (workbook, stockSummary) => {
    // Add the worksheet as the FIRST sheet
    // Headers: combine Quantity and Unit into a single "Quantity" column; remove total quantity sum row
    const worksheet = workbook.addWorksheet('Stock Summary', { properties: { tabColor: { argb: 'FFCCCC' } }, state: 'visible' });

    // Table headers: merged "Quantity" and "Unit" to a single "Quantity" column
    const headers = ['Sl No', 'Make', 'Quantity', 'Amount (Rs)'];

    worksheet.mergeCells('A1:D1');
    worksheet.getCell('A1').value = `Stock Summary Report`;
    worksheet.getCell('A1').font = { bold: true, size: 16 };
    worksheet.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' };
    worksheet.mergeCells('A2:D2');
    const today = new Date();
    worksheet.getCell('A2').value = `Date: ${today.toLocaleDateString('en-GB')}`;
    worksheet.getCell('A2').alignment = { vertical: 'middle', horizontal: 'center' };

    // Add the headers at row 4
    worksheet.addRow([]);
    ...
    // Add data rows with merged quantity and unit in one column
    let totalAmount = 0;

    stockSummary.forEach((row, idx) => {
      const amt = Number(row.Amount || 0);
      totalAmount += amt;
      const qty = row.total_qty !== undefined && row.Unit
        ? `${row.total_qty} ${row.Unit}`
        : (row.total_qty !== undefined ? row.total_qty : '');
    ...
    // No total quantity or "Grand Total" row with quantity.
    worksheet.addRow([]);
    const lastRowIdx = stockSummary.length + headerRowIdx + 1;
    worksheet.addRow([
      '',
      'Grand Total',
      '',
    ...
    const grandTotalRow = worksheet.getRow(lastRowIdx + 1);
    grandTotalRow.getCell(2).font = { bold: true, size: 13 };
    grandTotalRow.getCell(4).font = { bold: true, size: 13 };
    grandTotalRow.getCell(2).alignment = { vertical: 'middle', horizontal: 'center' };
    grandTotalRow.getCell(4).alignment = { vertical: 'middle', horizontal: 'center' };
    for (let c = 1; c <= headers.length; c++) {
    ...
    // Set column widths (reduced by one because unit column removed)
    const columnWidths = [8, 30, 18, 22];
    headers.forEach((header, idx) => {
      worksheet.getColumn(idx + 1).width = columnWidths[idx];
    });
  };
  // ------------------ END NEW STOCK SUMMARY ----------------------

  // Helper to generate a worksheet for a make, grouping products by category
  // Modified to hide rows having 0 quantity. User can unhide in Excel.
  const addMakeWorksheet = (workbook, makeName, data, headers) => {
    ...
    Object.keys(grouped).forEach((category, catIdx) => {
      ...
      grouped[category].forEach((row, idx) => {
        ...
        // Hide this product row if its quantity is 0/null/undefined or zero-like string
        const shouldHide =
          row.stock_qty === 0 ||
          row.stock_qty === '0' ||
          row.stock_qty === undefined ||
          row.stock_qty === null;
        if (shouldHide) {
        ...
        addedRow.getCell(8).alignment = { vertical: 'middle', horizontal: 'center' };
      });
    ...
    grandTotalRow.getCell(8).alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      data.forEach((row, rowIdx) => {
        ...
        switch (header) {
          ...
          case 'Cost Price':
            ...
            break;
          case 'STOCK':
            val = row.stock_qty !== undefined && row.Unit
              ? `${row.stock_qty} ${row.Unit}`
              : (row.stock_qty !== undefined ? row.stock_qty : '');
            break;
          case 'Total Price':
  ...
  // Handle Excel Export
  // Modified to hide rows with 0 quantity in filtered report as well
  const handleExcel = async () => {
    ...
    if (reportType === 'filtered' && !selectedMakeId) {
      Swal.fire({
        ...
        text: 'Please select a Make before exporting the filtered stock report.',
    ...
    try {
      const headers = [
        'SL No', 'Product Description', 'Model No', 'HSN / SAC Code', 'Cost Price', 'STOCK', 'Total Price'
      ];
      ...
      let excelFileName = '';
      if (reportType === 'full') {
        excelFileName = `BMP STOCK ${todayDateStr}.xlsx`;
      } else {
        const makeName = getMakeName(selectedMakeId) || '';
        const safeMakeName = makeName.replace(/[\\/:*?"<>|]/g, '');
        excelFileName = `BMP STOCK ${todayDateStr} - ${safeMakeName}.xlsx`;
      }

      // ALWAYS: fetch stock summary and add as first sheet if full report
      ...
      if (reportType === 'full') {
        try {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Stock Summary error',
            text: 'Failed to fetch stock summary. The Excel file will not contain the summary sheet.',
            confirmButtonColor: '#1976d2'
          });
          // Continue without stock summary
        }
      }
      ...
      if (reportType === 'full') {
        ...
        // 1. Add Stock Summary Sheet as FIRST
        if (stockSummaryData && stockSummaryData.length > 0) {
          addStockSummarySheet(workbook, stockSummaryData);
        } else {
          // Add empty summary sheet so the file always has the first "Stock Summary" sheet
          addStockSummarySheet(workbook, []);
        }
        ...
        // Filtered Stock Report (by make/category)
        const params = {};
        ...
        // Workbook/worksheet as before (no stock summary sheet for filtered)
        const workbook = new ExcelJS.Workbook();
  ...
  return (
    <div style={{ maxWidth: 400, margin: '40px auto', padding: 24, border: '1px solid #eee', borderRadius: 8, background: '#fff' }}>
      <h2 style={{ textAlign: 'center', marginBottom: 32 }}>Daily Stock Report</h2>
